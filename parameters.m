...
p.syn_popz=0.1*theta(8);%1.5;% popZ monomer synthesis
    ...
    p.deg_popzp=p.deg_popzm;%theta(9);%0.05;
    p.dnv_popz=40;%0.7*theta(10);%60;% denovo polymerization
    p.aut_popz=10;%3*theta(11);%3;% autocatalytic polymerization
     p.aut_popz1=0.7*p.aut_popz;
    p.alpha_PopZPodJ=120*theta(12);%1;%PodJ on PopZ
    p.depol_popz=0.5*theta(13);%0.1; % deplymerization 
    % diffussion rates
    p.D_popzm =835;%750; % PopZ monomer dffusion  
p.D_popzp =0.0005;%40;%0.0005;  % PopZ polymer diffusion
...
p.syn_divK1=0.0004;%0.0004;%Bronson
p.syn_divK2=0.05*3;%0.125;%Bronson
p.Ja_DivKCtrA=1.5;%1.7/2;%Bronson
p.deg_divK=0.014*3;%Bronson; 0.002;%Shenghua2008-exp
p.phoDivK=0.01;
p.alpha_DivKDivJ=1000;
p.dephoDivK=1;
p.D_DivK=1319;
p.D_DivKP=p.D_DivK;%p.D_DivK;
...
p.D_spmxp=0.0005;
%% CtrA
p.syn_ctrA1=0.008*2;%0.0083;%Shenghua2008-exp; 0.026;%Murray-fitted
p.Ji_CtrACtrA=3;
p.syn_ctrA2=0.073*2;%0.073;%Shenghua2008-exp
p.Ja_CtrACtrA=3;
p.deg_ctrA1=0.0038;%0.0038;%exp-calculated
p.deg_ctrA2=0.1;%0.1 exp-estimated
p.Jd_CtrA=0.05;
p.dephoCtrA=100000*log(2)/8;%Murray-estimated~0.14
p.phoCtrA=100000*0.03;%7/93*p.dephoCtrA/0.5%~0.021;%Murray-estimated
p.D_CtrA=427;
...
%% DivK
p.syn_divK1=0.0004;%0.0004;%Bronson
p.syn_divK2=0.05*5;%0.125;%Bronson
p.Ja_DivKCtrA=1.5;%1.7/2;%Bronson
p.deg_divK=0.014;%Bronson; 0.002;%Shenghua2008-exp
p.phoDivK=0.01;
p.alpha_DivKDivJ=100;
p.dephoDivK=1;
p.D_DivK=1319;
p.D_DivKP=p.D_DivK;%p.D_DivK;
